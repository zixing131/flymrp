...
it('retains the browser shell and sends handset files and their indexes to Blob', () => {
  for (const name of ['index.html','main.html','about.html','manifest.json','build-version.json','sw.js','assets/app.js','icons/icon-192.png','licenses/audio.txt']) expect(isRuntimeAsset(name)).toBe(false);
